...
入口 `https://47.250.10.235:8080`,应用本身只绑 127.0.0.1:3015,
...
```
https://47.250.10.235:8080/
```

**已实测可达**,凭据是 `.env` 里的 `AUTH_USER` / `AUTH_PASS`(HTTP Basic)。
浏览器会因自签证书警告一次,点继续即可。

| 层 | 配置 |
|---|---|
| 应用 | uvicorn 绑 `127.0.0.1:3015`,**从不**绑 0.0.0.0 |
| 代理 | Caddy 监听 `:8080` 和 `:8443`,两个都是 TLS,终结后转发到回环 |
...
443 不可用,被 xray(VPN)占着。8443 是常规选择但**阿里云安全组没放行**。

于是逐个端口探了一遍安全组:在本机 curl 实例自己的公网 IP **会经过安全组**,
已放行的端口正常应答,未放行的**超时**(不是 connection refused)。
探测结果 3016、3020、3001、8000、9000 全部超时,**只有 8080 已放行**。
所以用 8080,零控制台操作。TLS 跑在 8080 上不寻常但没有任何问题,scheme 在 URL 里写明即可。

`:8443` 也一直在监听,等哪天安全组放行了直接可用。
...
"connection refused" 失败,而配置本身校验是通过的。保留 `admin off` 值得这点代价:
少一个能重写本代理配置的本地接口。
