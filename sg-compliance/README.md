...
- **Phase 3 ✅** — AI 助手：`/[locale]/chat` 客户端流式对话，`/api/chat` 通过 `lib/ai/llm.ts` 调用 GLM-5.1（腾讯 Token Plan，OpenAI 兼容，base `https://api.lkeap.cloud.tencent.com/plan/v3`），不做 RAG —— 把全部合规模块内容（约 5-8K tokens）+ 最近 30 条新闻一次性塞进 system prompt。需在 `/etc/sg-compliance/env` 配置 `TOKENPLAN_API_KEY`（`sk-tp-...`）。
